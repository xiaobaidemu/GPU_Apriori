...
BenMM_t *BenStartMMap(char *fileName)
{
	...
	// char *lpbMapAddress = (char *)MapViewOfFile(hFileMap,FILE_MAP_ALL_ACCESS,0, 0,dwBlockBytes);
	BEN_ASSERT(lpbMapAddress != NULL);
	//printf("\n!!!!test:lpbMapAddress[i]=%d,%d,%d,%d,%d,%d,%d",lpbMapAddress[0],lpbMapAddress[1],lpbMapAddress[2],lpbMapAddress[3],lpbMapAddress[4],lpbMapAddress[5],lpbMapAddress[6]);
